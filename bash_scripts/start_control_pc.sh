...
start_franka_ft_on_control_pc_path="$DIR/start_franka_gripper_on_control_pc.sh"
echo "Will ssh to control PC and start ROS gripper node."
gnome-terminal --working-directory="$DIR" -- bash $start_franka_gripper_on_control_pc_path $control_pc_uname $control_pc_ip_address $workstation_ip_address $control_pc_franka_interface_path $ft_ip $robot_ip $control_pc_use_password $control_pc_password
sleep 3
else
    echo "Will not start franka gripper on the control pc."
fi
